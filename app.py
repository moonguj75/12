...
st.set_page_config(page_title="부부 은퇴 자산 입체 계측기 v6.0", layout="wide")
...
st.title("👑 부부 은퇴 자산 초정밀 계측기 v6.0")
st.caption("저축 항목 수동 기입 기능 및 선택 연차 타겟 시뮬레이션 시스템")
st.markdown("---")
...
# 1단계: [★대변신★] 연차별 자산 설정 및 저축액 수동 기입 구역
st.sidebar.markdown("---")
st.sidebar.header("⚙️ 연차별 자산 및 저축 설정")

# 사용자가 보고 싶고, 세팅하고 싶은 "특정 연차"를 딱 하나만 선택하도록 고정!
...
for y in range(1, years_to_run + 1):
    if y not in st.session_state.asset_config:
        if y == 1:
            ...
            st.session_state.asset_config[y] = {"h_jesus": 0, "h_cma": 1350, "w_deposit": 0, "w_cma": 0, "w_new_deposit": 0, "manual_save": 0, "save_dest": "👩 아내 CMA"}
...
# [★핵심 추가★] 이번 연도에 내가 직접 타이핑해서 밀어 넣을 저축액 수동 기입창
st.sidebar.markdown("---")
...
if st.button(f"🚀 {selected_setup_year} 자산 장부 집중 동기화 가동", type="primary"):
    ...
    for year in range(1, years_to_run + 1):
        ...
        for month in range(1, 13):
            ...
            # [★필터링★] 사용자가 선택한 연차의 월별 데이터만 뼈대 추려내기
        ...
        # [★수동 저축 메커니즘 연동★] 사용자가 직접 기입한 금액과 주머니 위치 반영
