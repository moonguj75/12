...
st.set_page_config(page_title="부부 은퇴 자산 입체 계측기 v6.2", layout="wide")
...
st.title("👑 부부 은퇴 자산 초정밀 계측기 v6.2")
st.caption("저축 항목 완전 제거로 에러 차단: 선택 연차 전용 클린 시뮬레이션 시스템")
st.markdown("---")
...
# 1단계: 연차별 자산 설정 구역 (저축 기입 칸 완벽 제거)
st.sidebar.markdown("---")
...
if st.button(f"🚀 {selected_setup_year} 자산 장부 집중 동기화 가동", type="primary"):
    ...
    with tab_yearly:
        ...
        with col_tot3:
            ...
            st.caption("※ 연말에 남은 잔돈은 자동으로 아내 CMA 통장으로 귀속되어 복리로 이어집니다.")
