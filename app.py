...
st.set_page_config(page_title="부부 은퇴 자산 입체 계측기 v6.5", layout="wide")
...
st.title("👑 부부 은퇴 자산 초정밀 계측기 v6.5")
st.caption("KeyError 영구 박멸 버전: 로직 단순화를 통한 완벽한 안정성 확보 시스템")
st.markdown("---")
...
# 초기값 데이터 구조 단순화 (KeyError 유발 원인 제거)
for y in range(1, years_to_run + 1):
    if y not in st.session_state.asset_config:
        if y == 1:
            ...
            st.session_state.asset_config[y] = {"h_jesus": 0, "h_cma": 1350, "w_deposit": 0, "w_cma": 0, "w_new_deposit": 0}
...
if st.button(f"🚀 {selected_setup_year} 자산 장부 집중 동기화 가동", type="primary"):
    ...
    for year in range(1, years_to_run + 1):
        ...
        # [★에러 원천 차단★] 복잡한 선택 구조 제거하고 남은 잔돈은 무조건 안전하게 아내 CMA로 수렴 이양
        w_cma += leftover
    ...
    with tab_yearly:
        ...
        with col_tot3:
            st.success(f"💰 통장 깨고 남은 알짜 잔돈(자동 저축)", f"{int(target_res['잔돈']/10000):,} 만원")
            st.caption("※ 연말에 남은 잔돈은 자동으로 가장 안전한 아내 CMA 통장으로 전액 이양되어 완벽한 복리로 불어납니다.")
