...
ac["w_deposit"] = st.sidebar.number_input(f"👩 {setup_year_num}년차 아내 기존정기예금 (만원)",  step=500,  value=int(ac["w_deposit"]), disabled=not ac["override"])
ac["w_cma"]     = st.sidebar.number_input(f"👩 {setup_year_num}년차 아내 CMA 잔액 (만원)",      step=50,   value=int(ac["w_cma"]),     disabled=not ac["override"])
...
with c_etf:
    ...
    cfg["tr_etf_rate"] = st.slider("지수 TR ETF 예상 연 수익률 (%)", 1.0, 10.0, step=0.5, value=float(cfg["tr_etf_rate"]))

with c_vars:
    st.subheader("💰 시장 적용 금리")
    cfg["deposit_rate"] = st.slider("저축은행 정기예금 금리 (%)", 1.0, 6.0, step=0.1, value=float(cfg["deposit_rate"]))
    cfg["cma_rate"]     = st.slider("대신증권 CMA 금리 (%)",      1.0, 6.0, step=0.1, value=float(cfg["cma_rate"]))
    cfg["jesus_rate"]   = st.slider("증권사 주식 예수금 이율 (%)", 0.1, 3.0, step=0.1, value=float(cfg["jesus_rate"]))

tr_etf_rate  = cfg["tr_etf_rate"]  / 100
deposit_rate = cfg["deposit_rate"] / 100
cma_rate     = cfg["cma_rate"]     / 100
...
if st.button(f"🚀 {setup_year_num}년차 ({start_year + setup_year_num - 1}년) 장부 집중 가동", type="primary", use_container_width=True):
    ...
    for year in range(1, years_to_run + 1):
        ...
        use_override = (year == 1) or ac_y.get("override", False)

        h_jesus   = ac_y["h_jesus"]   * 10000       if use_override else carry_h_jesus
        h_deposit = ac_y["h_deposit"] * 10000       if use_override else carry_h_deposit
        h_cma     = ac_y["h_cma"]     * 10000       if use_override else carry_h_cma
        w_deposit = ac_y["w_deposit"] * 10000       if use_override else carry_w_deposit
        w_cma     = ac_y["w_cma"]     * 10000       if use_override else carry_w_cma

        h_jesus_monthly_balances = []
        h_deposit_init = h_deposit
        h_cma_init     = h_cma
        ...
        for month in range(1, 13):
            ...
            if year == setup_year_num:
                ...
                monthly_records.append({
                    ...
                    "👩아내 정기예금":          int(max(0, w_deposit) / 10000),
        ...
        yearly_records.append({
            ...
            "부족분": deficit,
            "잔돈": leftover,
            "남편예수금_avg": avg_h_jesus,
            "남편예금_val": h_deposit_init,
            "남편CMA_val": h_cma_init,
            "아내예금_val": w_deposit_init,
            "아내CMA_val": w_cma_init
        })
    ...
    with tab_yearly:
        ...
        with col_w:
            ...
            if w_margin > 0:
                ...
                st.error("🚨 경고: 2,000만원 초과! 종합과세 대상")

        st.markdown("---")
        st.subheader("🧾 부부 합산 세후 이자 산출 명세서")
        receipt_df = pd.DataFrame([
            {"項目 明細": "👨 남편 명의 세전 금융소득 합계",         "금액": f"{int(target_res['남편세전']/10000):,} 만원"},
            {"項目 明細": "👩 아내 명의 세전 금융소득 합계",         "금액": f"{int(target_res['아내세전']/10000):,} 만원"},
            {"項目 明細": "📊 부부 세전 소득 합계 (A)",              "금액": f"{int(target_res['부부세전합계']/10000):,} 만원"},
            {"項目 明細": "💸 이자소득세 원천징수 (A × 15.4%)",      "금액": f"- {int((target_res['부부세전합계']*0.154)/10000):,} 만원"},
            {"項目 明細": "👪 부부 최종 세후 이자 수령액",           "금액": f"{int(target_res['세후이자']/10000):,} 만원"},
        ])
        st.table(receipt_df)
        ...
        with col_tot1:
            st.metric("👪 부부 합산 세후 이자", f"{int(target_res['세후이자']/10000):,} 만원")
        with col_tot2:
            st.warning(f"🛒 연간 생활비 구멍: {int(target_res['부족분']/10000):,} 만원")
        with col_tot3:
            st.success(f"💰 통장 깨고 남은 잔돈(자동 저축): {int(target_res['잔돈']/10000):,} 만원")
