...
st.set_page_config(page_title="부부 은퇴 자산 입체 계측기 v6.4", layout="wide")
...
st.title("👑 부부 은퇴 자산 초정밀 계측기 v6.4")
st.caption("KeyError 완벽 패치: 데이터 로딩 안정성 확보 및 선택 연차 집중 계측 시스템")
st.markdown("---")
...
dest_options = ["👩 아내 CMA", "👨 남편 CMA", "📈 지수 TR ETF"]

# [★패치 완료★] 모든 연차의 기본 데이터 구조를 누락 없이 100% 강제 생성
for y in range(1, years_to_run + 1):
    if y not in st.session_state.asset_config:
        if y == 1:
            st.session_state.asset_config[y] = {"h_jesus": 110000, "h_cma": 1350, "w_deposit": 42000, "w_cma": 1200, "w_new_deposit": 0, "save_dest": "👩 아내 CMA"}
        elif y == 2:
            st.session_state.asset_config[y] = {"h_jesus": 55000, "h_cma": 1350, "w_deposit": 36500, "w_cma": 4000, "w_new_deposit": 8000, "save_dest": "👩 아내 CMA"}
        else:
            st.session_state.asset_config[y] = {"h_jesus": 0, "h_cma": 1350, "w_deposit": 0, "w_cma": 0, "w_new_deposit": 0, "save_dest": "👩 아내 CMA"}
    else:
        # 혹시 기존 세션에 save_dest가 누락되어 있다면 안전하게 채워넣음 (KeyError 방지)
        if "save_dest" not in st.session_state.asset_config[y]:
            st.session_state.asset_config[y]["save_dest"] = "👩 아내 CMA"
...
# [★안전장치★] .get() 함수를 사용하여 값이 없더라도 에러 없이 기본값 처리
current_dest = st.session_state.asset_config[setup_year_num].get("save_dest", "👩 아내 CMA")
if current_dest not in dest_options:
    current_dest = "👩 아내 CMA"
default_idx = dest_options.index(current_dest)

st.sidebar.markdown("---")
st.sidebar.subheader(f"🔄 [{selected_setup_year}] 만기 잔돈 귀속처")
c_sd = st.sidebar.selectbox("연말 잔돈을 보낼 주머니", dest_options, index=default_idx)

st.session_state.asset_config[setup_year_num] = {
    "h_jesus": c_hj, "h_cma": c_hc, "w_deposit": c_wd, "w_new_deposit": c_wn, "w_cma": c_wc, "save_dest": c_sd
}
...
if st.button(f"🚀 {selected_setup_year} 자산 장부 집중 동기화 가동", type="primary"):
    ...
    with tab_yearly:
        ...
        with col_tot3:
            st.success(f"💰 통장 깨고 남은 알짜 잔돈", f"{int(target_res['잔돈']/10000):,} 만원")
            st.caption(f"※ 연말 잔돈은 지정하신 【{target_res['저축처']}】 주머니로 안전하게 이양되어 복리로 굴러갑니다.")
