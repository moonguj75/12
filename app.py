...
st.set_page_config(page_title="부부 은퇴 자산 입체 계측기 v6.1", layout="wide")
...
st.title("👑 부부 은퇴 자산 초정밀 계측기 v6.1")
st.caption("에러 방지 엔진 탑재: 연차별 자산 입력 및 저축 흐름 무결점 정산 시스템")
st.markdown("---")
...
# 1단계: 연차별 자산 설정 및 저축액 수동 기입 구역
st.sidebar.markdown("---")
st.sidebar.header("⚙️ 연차별 자산 및 저축 설정")

# 사용자가 보고 싶고, 세팅하고 싶은 "특정 연차"를 선택
...
for y in range(1, years_to_run + 1):
    if y not in st.session_state.asset_config:
        if y == 1:
            st.session_state.asset_config[y] = {"h_jesus": 110000, "h_cma": 1350, "w_deposit": 42000, "w_cma": 1200, "w_new_deposit": 0, "manual_save": 0, "save_dest": "👩 아내 CMA"}
        elif y == 2:
            st.session_state.asset_config[y] = {"h_jesus": 55000, "h_cma": 1350, "w_deposit": 36500, "w_cma": 4000, "w_new_deposit": 8000, "manual_save": 0, "save_dest": "👩 아내 CMA"}
        else:
            st.session_state.asset_config[y] = {"h_jesus": 0, "h_cma": 0, "w_deposit": 0, "w_cma": 0, "w_new_deposit": 0, "manual_save": 0, "save_dest": "👩 아내 CMA"}
...
st.sidebar.markdown("---")
st.sidebar.subheader(f"💰 [{selected_setup_year}] 저축 및 재투자 직접 기입")
c_ms = st.sidebar.number_input(f"⊙ {selected_setup_year}에 추가로 저축할 금액 (만원)", value=st.session_state.asset_config[setup_year_num]["manual_save"], step=100)
c_sd = st.sidebar.selectbox(f"⊙ {selected_setup_year} 저축액을 보낼 주머니", ["👩 아내 CMA", "👨 남편 CMA", "📈 지수 TR ETF"], index=["👩 아내 CMA", "👨 남편 CMA", "📈 지수 TR ETF"].index(st.session_state.asset_config[setup_year_num]["save_dest"]))

st.session_state.asset_config[setup_year_num] = {
    "h_jesus": c_hj, "h_cma": c_hc, "w_deposit": c_wd, "w_new_deposit": c_wn, "w_cma": c_wc, "manual_save": c_ms, "save_dest": c_sd
}
...
if st.button(f"🚀 {selected_setup_year} 자산 장부 집중 동기화 가동", type="primary"):
    ...
    # 1년부터 차례대로 순차 연산 수행 (에러 방지를 위해 이월 로직 고정)
    for year in range(1, years_to_run + 1):
        cfg = st.session_state.asset_config[year]
        
        # 사용자가 입력 창에 0을 그대로 두면 전년도 금액을 이월받고, 값을 따로 고쳤으면 고친 값을 강제 주입
        h_jesus = (cfg["h_jesus"] * 10000) if (year == 1 or cfg["h_jesus"] > 0) else carry_h_jesus
        h_cma = (cfg["h_cma"] * 10000) if (year == 1 or cfg["h_cma"] > 0) else carry_h_cma
        w_deposit = (cfg["w_deposit"] * 10000) if (year == 1 or cfg["w_deposit"] > 0) else carry_w_deposit
        w_new_deposit = (cfg["w_new_deposit"] * 10000) if (year == 1 or cfg["w_new_deposit"] > 0) else carry_w_new_deposit
        w_cma = (cfg["w_cma"] * 10000) if (year == 1 or cfg["w_cma"] > 0) else carry_w_cma
        ...
        # 수동 기입 저축액 처리
        user_save_amount = cfg["manual_save"] * 10000
        dest = cfg["save_dest"]
        
        if "아내 CMA" in dest:
            w_cma += user_save_amount
        elif "남편 CMA" in dest:
            h_cma += user_save_amount
        elif "지수 TR ETF" in dest:
            h_tr_etf += user_save_amount

        # 다음 해를 위해 연말 잔고 보관 (이월 엔진)
    ...
    with tab_yearly:
        ...
        with col_tot3:
            st.metric(f"✍️ 내가 직접 기입한 수동 저축액", f"{int(target_res['수동저축액']/10000):,} 만원")
            st.caption(f"※ 설정하신 대로 【{target_res['저축처']}】 주머니로 안전하게 이양되었습니다.")
